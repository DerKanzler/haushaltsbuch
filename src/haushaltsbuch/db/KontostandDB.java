...
public class KontostandDB {
	...
	public Vector<Kontostand> getKontostaendeMonat(Date date) throws Exception {
		...
		while (rs.next()) {
			...
			//if (ktost.getKonto().getAbldat().after(ktost.getKtostdat()) || ktost.getKtostsaldo().compareTo(new BigDecimal(0)) != 0) kontostaende.addElement(ktost);
	...
	public void save(Kontostand kontostand) throws Exception {
		...
		ps.setDate(1, new Date(kontostand.getKtostdat().getTime()));
	...
	private Kontostand resultSetToKontostand(ResultSet rs) throws Exception {
		Kontostand kontostand = new Kontostand();
		kontostand.setKtost(rs.getInt("ktost#"));
		kontostand.setKtostdat(rs.getDate("ktostdat"));
