...
public class BuchungDB {
	...
	public void save(Buchung buchung) throws Exception {
		...
		ps.setDate(4, new Date(buchung.getEindat().getTime()));
		if (buchung.getKoart() == null) {
			ps.setNull(5, Types.INTEGER);
		}
		else ps.setInt(5, buchung.getKoart().getKoart());
		if (buchung.getKontovon() == null) {
			ps.setNull(6, Types.INTEGER);
		}
		else ps.setInt(6, buchung.getKontovon().getKonto());
		if (buchung.getKontonach() == null) {
			ps.setNull(7, Types.INTEGER);
		}
		else ps.setInt(7, buchung.getKontonach().getKonto());
		ps.execute();
		ps.close();
	}
	...
	public Date getFirstBookDate() throws Exception {
		...
		PreparedStatement ps = DB.instance().getConnection().prepareStatement("SELECT MIN(\"buchdat\") AS \"firstBookDate\" FROM \"tbuchung\"");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public Date getLastBookDate() throws Exception {
		Date lastBookDate = null;
		PreparedStatement ps = DB.instance().getConnection().prepareStatement("SELECT MAX(\"buchdat\") AS \"lastBookDate\" FROM \"tbuchung\"");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			lastBookDate = rs.getDate("lastBookDate");
		}
		ps.close();
		return lastBookDate;
	}
	...
	private Buchung resultSetToBuchung(ResultSet rs) throws SQLException {
		...
		buchung.setBuchdat(rs.getDate("buchdat"));
		buchung.setBuchtext(rs.getString("buchtext"));
		buchung.setEindat(rs.getDate("eindat"));
