...
import haushaltsbuch.Haushaltsbuch;
...
public class DBConfiguration {
	...
	private void readConfiguration() throws ConfigurationException {
		try {
			path = Preferences.userNodeForPackage(Haushaltsbuch.class).node(LogicMain.instance().getProfile()).get("path", "");
			if (path == null || path.isEmpty()) throw new ConfigurationException();
			name = Preferences.userNodeForPackage(Haushaltsbuch.class).node(LogicMain.instance().getProfile()).get("name", "");
			if (name == null || name.isEmpty()) throw new ConfigurationException();
		}
	...
	public void writeConfiguration(String file) {
		...
		try {
			Preferences pref = Preferences.userNodeForPackage(Haushaltsbuch.class).node(LogicMain.instance().getProfile());
			pref.put("path", path);
			pref.put("name", name);
			pref.flush();
		}
