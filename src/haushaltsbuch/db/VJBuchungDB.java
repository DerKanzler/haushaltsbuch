...
public class VJBuchungDB {
	...
	public void save(Buchung buchung) throws Exception {
		...
		ps.setDate(2, new Date(buchung.getBuchdat().getTime()));
		ps.setString(3, buchung.getBuchtext());
		if (buchung.getKoart() == null) {
			ps.setNull(4, Types.INTEGER);
		}
		else ps.setInt(4, buchung.getKoart().getKoart());
		if (buchung.getKontovon() == null) {
			ps.setNull(5, Types.INTEGER);
		}
		else ps.setInt(5, buchung.getKontovon().getKonto());
		if (buchung.getKontonach() == null) {
			ps.setNull(6, Types.INTEGER);
		}
		else ps.setInt(6, buchung.getKontonach().getKonto());
		ps.execute();
		ps.close();
	}
	...
	public Date getFirstBookDate() throws Exception {
		Date firstBookDate = null;
		PreparedStatement ps = DB.instance().getConnection().prepareStatement("SELECT MIN(\"vjbuchdat\") AS \"firstBookDate\" FROM \"tvjbuchung\"");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			firstBookDate = rs.getDate("firstBookDate");
		}
		ps.close();
		return firstBookDate;
	}
	
	public Date getLastBookDate() throws Exception {
		Date firstBookDate = null;
		PreparedStatement ps = DB.instance().getConnection().prepareStatement("SELECT MAX(\"vjbuchdat\") AS \"lastBookDate\" FROM \"tvjbuchung\"");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			firstBookDate = rs.getDate("lastBookDate");
		}
		ps.close();
		return firstBookDate;
	}
	...
	private Buchung resultSetToBuchung(ResultSet rs) throws Exception {
		...
		buchung.setBuchdat(rs.getDate("vjbuchdat"));
	...
	private String createWhere(SearchBuchung suchBuchung) {
		...
		if (suchBuchung.getToAccount() != null) {
			if (count > 0) {
				...
				else where = where.concat("AND ");
			}
			where = where.concat("\"kontonach#\" = ?");
			if (suchBuchung.isSameAccount()) {
				where = where.concat(") ");
			}
			else where = where.concat(" ");
			count++;
		}
	...
	private PreparedStatement prepareStatement(PreparedStatement ps, SearchBuchung suchBuchung) throws Exception {
		Integer count = 1;
		if (suchBuchung.getBookText() != null) {
		...
		if (suchBuchung.getFromDate() != null) {
			ps.setDate(count, new Date(suchBuchung.getFromDate().getTime()));
			count++;
		}
		if (suchBuchung.getToDate() != null) {
			ps.setDate(count, new Date(suchBuchung.getToDate().getTime()));
			count++;
		}
		if (suchBuchung.getAmount() != null) {
