...
public class DB {
	...
	public Boolean connect() throws InstallationException, ConfigurationException {
		try {
			...
			connection = DriverManager.getConnection(url.concat(dbconfig.getDBPath()).concat(dbconfig.getDBName()).concat(";IFEXISTS=TRUE"), user, pwd);
	...
	private Boolean createDatabase() throws InstallationException, ConfigurationException {
		try {
			...
			DBConfiguration dbconfig = new DBConfiguration();
			Class.forName(driver);
			connection = DriverManager.getConnection(url.concat(dbconfig.getDBPath()).concat(dbconfig.getDBName()), user, pwd);
			connection.setAutoCommit(false);
			connected = true;
