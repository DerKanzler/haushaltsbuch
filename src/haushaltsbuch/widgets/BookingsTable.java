...
public class BookingsTable implements SelectionListener {
	...
	public BookingsTable(Composite parent, Integer span) {
		...
		String[] columnNames = {res.getString("Date"), res.getString("BookingText"), res.getString("Amount"), res.getString("Costtype"), res.getString("FromAccount"), res.getString("ToAccount")};
		for (int i = 0; i < columnNames.length; i++) {
		      TableColumn column = new TableColumn(table, SWT.LEFT);
		      column.setText(columnNames[i]);
		      if (i>1) column.setAlignment(SWT.RIGHT);
		      if (i==1) {
		    	  layout.setColumnData(column, new ColumnWeightData(360));
		      }
		      else layout.setColumnData(column, new ColumnWeightData(100));
	...
	public void widgetSelected(SelectionEvent se) {
		if (se.getSource().equals(table.getColumn(0))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.BUCHDAT_UP);
			else setSorting(BookingsSorter.BUCHDAT_DOWN);
		}
		else if (se.getSource().equals(table.getColumn(1))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.BUCHTEXT_UP);
			else setSorting(BookingsSorter.BUCHTEXT_DOWN);	
		}
		else if (se.getSource().equals(table.getColumn(2))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.BUCHBETR_UP);
			else setSorting(BookingsSorter.BUCHBETR_DOWN);			
		}
		else if (se.getSource().equals(table.getColumn(3))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.KOART_UP);
			else setSorting(BookingsSorter.KOART_DOWN);			
		}
		else if (se.getSource().equals(table.getColumn(4))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.KONTOVON_UP);
			else setSorting(BookingsSorter.KONTOVON_DOWN);			
		}
		else if (se.getSource().equals(table.getColumn(5))) {
			if (table.getSortDirection() == SWT.DOWN) setSorting(BookingsSorter.KONTONACH_UP);
			else setSorting(BookingsSorter.KONTONACH_DOWN);	
		}
	}
