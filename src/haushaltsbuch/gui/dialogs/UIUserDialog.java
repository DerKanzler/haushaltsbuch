...
import haushaltsbuch.logic.dialogs.LogicBenutzer;
...
public class UIUserDialog extends IconAndMessageDialog {
	...
	private Benutzer readFields() throws RuntimeException {
		...
		else throw new RuntimeException("Name muss angegeben werden!");
		if (password1Text.getText().length() > 0 && password2Text.getText().length() > 0) {
			...
			else throw new RuntimeException("Das Passwort muss übereinstimmen!");
		}
		else throw new RuntimeException("Das Passwort muss zweimal angegeben werden!");
		if (formatCombo.getSelectedAccount() != null) {
			b.setFormat(formatCombo.getSelectedAccount());
		}
		else throw new RuntimeException("Das Format für das Datum, Beträge, etc. muss angegeben werden!");
		return b;
	}
