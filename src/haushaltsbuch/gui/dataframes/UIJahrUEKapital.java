...
public class UIJahrUEKapital extends UIAbstractDataFrame implements SelectionListener, ISelectionChangedListener {
	...
	public UIJahrUEKapital(UIDataFrame parent, Integer style) {
		...
		bookingsButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event e) {
				...
				new UIKapitalKontoDialog(Display.getDefault().getActiveShell(), UIKapitalDialog.KAPITAL_BOOKINGS, k).open();
			}
		...
		detailsButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event e) {
				KontoCollection k = table.getSelectedAccount();
				new UIKapitalKontoDialog(Display.getDefault().getActiveShell(), UIKapitalDialog.KAPITAL_CHART, k).open();
			}
	...
	private void buildSummaryGroup(Group group) {
		...
		dispChartButton.setImage(new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/chart.png")));
		...
		lockChartButton.setImage(new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/chart.png")));
		...
		sumChartButton.setImage(new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/chart.png")));
	...
	private void fillFields(Integer year) {
		...
		else yearText.setMaximum(Tools.getYear(Tools.getLastDate()));
		...
		if (logic.getDispDiff().compareTo(new BigDecimal(0)) < 0) dispDiffLabel.setForeground(red);
		else if (logic.getDispDiff().compareTo(new BigDecimal(0)) > 0) dispDiffLabel.setForeground(green);
		else dispDiffLabel.setForeground(black);
		...
		if (logic.getLockDiff().compareTo(new BigDecimal(0)) < 0) lockDiffLabel.setForeground(red);
		else if (logic.getLockDiff().compareTo(new BigDecimal(0)) > 0) lockDiffLabel.setForeground(green);
		else lockDiffLabel.setForeground(black);
		...
		if (logic.getSumDiff().compareTo(new BigDecimal(0)) < 0) sumDiffLabel.setForeground(red);
		else if (logic.getSumDiff().compareTo(new BigDecimal(0)) > 0) sumDiffLabel.setForeground(green);
		else sumDiffLabel.setForeground(black);
