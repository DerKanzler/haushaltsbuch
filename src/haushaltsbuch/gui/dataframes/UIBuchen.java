...
public class UIBuchen extends UIAbstractDataFrame implements SelectionListener, IDoubleClickListener, ISelectionChangedListener {
	...
	private Label bookMonthLabel, accountOneLabel, accountTwoLabel, accountThreeLabel, accountOneData, accountTwoData, accountThreeData, disposableAmountData, offlimitAmountData, totalAssetsData, totalRevenuesData, totalExpensesData, totalSavingsData, messageLabel;
	...
	private void buildBookingGroup(Group group) {
		...
		dateText.setSelection(Tools.getLastDate());
	...
	public void activate() {
		...
		if (focusedControl == null) dateText.setFocus();
		else focusedControl.setFocus();
	}
	...
	private void fillFields() {
		...
		if (user.getKonto1() != null) {
			...
			if (user.getKonto1().getSaldo().compareTo(new BigDecimal(0)) < 0) accountOneData.setForeground(red);
			else accountOneData.setForeground(black);
		}
		else {
		...
		if (user.getKonto2() != null) {
			...
			if (user.getKonto2().getSaldo().compareTo(new BigDecimal(0)) < 0) accountTwoData.setForeground(red);
			else accountTwoData.setForeground(black);
		}
		else {
		...
		if (user.getKonto3() != null) {
			...
			if (user.getKonto3().getSaldo().compareTo(new BigDecimal(0)) < 0) accountThreeData.setForeground(red);
			else accountThreeData.setForeground(black);
		}
		else {
		...
		if (logic.getDisposableSum().compareTo(new BigDecimal(0)) < 0) disposableAmountData.setForeground(red);
		else disposableAmountData.setForeground(black);
		
		offlimitAmountData.setText(Tools.printBigDecimal(logic.getOffLimitSum()) + " €");
		if (logic.getOffLimitSum().compareTo(new BigDecimal(0)) < 0) offlimitAmountData.setForeground(red);
		else offlimitAmountData.setForeground(black);
		
		totalAssetsData.setText(Tools.printBigDecimal(logic.getTotalAssets()) + " €");
		if (logic.getTotalAssets().compareTo(new BigDecimal(0)) < 0) totalAssetsData.setForeground(red);
		else totalAssetsData.setForeground(black);
		...
		if (logic.getTotalSavings().compareTo(new BigDecimal(0)) < 0) totalSavingsData.setForeground(red);
		else if (logic.getTotalSavings().compareTo(new BigDecimal(0)) == 0) totalSavingsData.setForeground(black);
		else totalSavingsData.setForeground(green);
	...
	private Buchung readFields() throws RuntimeException {
		...
		if (dateText.getSelection() != null) {
			b.setBuchdat(dateText.getSelection());
		}
		else throw new RuntimeException("Das Datum darf nicht leer sein!");
		if (!bookingText.getText().equals("")) {
			b.setBuchtext(bookingText.getText());
		}
		else throw new RuntimeException("Der Buchungstext darf nicht leer sein!");
		if (amountText.getValue() instanceof Integer) {
			...
			else throw new RuntimeException("Der Betrag muss größer als 0 sein!");
		}
	...
	private void save() {
		try {
			...
			else messageLabel.setText("Das Speichern ist fehlgeschlagen!");
		}
	...
	/**
	 * Opens the dialog to edit the booking text of the selected booking.
	 * Updates the bookingsTable then afterwards.
	 */
	private void edit() {
	...
	private void delete() {
		try {
			...
			else messageLabel.setText("Das Löschen ist fehlgeschlagen!");
		}
		catch (RuntimeException re) {
			messageLabel.setText(re.getMessage());
		}
	...
	/**
	 * Catches the double clicks on a row of the table and calls the private edit method.
	 */
	...
	private String printYearEnds(Vector<Date> dates) {
		StringBuffer sb = new StringBuffer();
		if (dates.size() > 1) {
			sb.append("Der Jahresabschluss für folgende Jahre ");
		}
		else sb.append("Der Jahresabschluss für das folgende Jahr ");
		sb.append("wurde erfolgreich durchgeführt:\n\n");
		for (int i=0; i<dates.size(); i++) {
			sb.append(new SimpleDateFormat("yyyy").format(dates.elementAt(i)) + "\n");
		}
		return sb.toString();
	}
	
	private String printMonthEnds(Vector<Date> dates) {
		StringBuffer sb = new StringBuffer();
		if (dates.size() > 1) {
			sb.append("Der Monatsabschluss für folgende Monate ");
		}
		else sb.append("Der Monatsabschluss für das folgende Monat ");
		sb.append("wurde erfolgreich durchgeführt:\n\n");
		for (int i=0; i<dates.size(); i++) {
			sb.append(new SimpleDateFormat("MMMM yyyy").format(dates.elementAt(i)) + "\n");
		}
		return sb.toString();
	}
