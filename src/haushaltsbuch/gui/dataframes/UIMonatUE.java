...
public class UIMonatUE extends UIAbstractDataFrame implements SelectionListener {
	...
	private Label disposableAmountData, offlimitAmountData, totalAssetsData, totalRevenuesData, totalExpensesData, totalSavingsData;
	...
	/**
	 * Constructor: Since UIMonatUE extends Composite the super constructor is called here with the given arguments.
	...
	public UIMonatUE(UIDataFrame parent, Integer style) {
		...
		monthText.setSelection(Tools.getLastMonth());
		monthText.addSelectionListener(this);
		
		previousMonthButton = new Button(this, SWT.FLAT);
		previousMonthButton.setImage(new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/back.png")));
		previousMonthButton.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END | GridData.FILL_HORIZONTAL | GridData.VERTICAL_ALIGN_CENTER));		
		previousMonthButton.addSelectionListener(this);
		
		nextMonthButton = new Button(this, SWT.FLAT);
		nextMonthButton.setImage(new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/forward.png")));
	...
	private void fillAccountsGroup(Vector<Kontostand> kost) {
		...
		for (Kontostand k: kost) {
			...
			if (k.getKtostsaldo().compareTo(new BigDecimal(0)) < 0) accountSum.setForeground(red);
			...
			accountEndDate.setText(new SimpleDateFormat("dd.MM.yyyy").format(k.getKonto().getAbldat()));
			
			if (count%4 != 0) new Label(accountsComposite, SWT.NONE).setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		}
	...
	public void widgetSelected(SelectionEvent se) {
		if (se.getSource().equals(previousMonthButton)) {
			...
			if (monthText.getSelection() != null) {
				GregorianCalendar gc = new GregorianCalendar();
				gc.setTime(monthText.getSelection());
				fillFields(new GregorianCalendar(gc.get(GregorianCalendar.YEAR), gc.get(GregorianCalendar.MONTH), 1).getTime());	
			}
		}
	}
	...
	@Override
	public void update() {
		fillFields(monthText.getSelection());
