 ...
 *
 */
public class UISuchen extends UIAbstractDataFrame implements SelectionListener, IDoubleClickListener, ISelectionChangedListener {
	...
	/**
	 * Constructor: Since UISuchen extends Composite the super constructor is called here with the given arguments.
	...
	public UISuchen(UIDataFrame parent, Integer style) {
		...
		// The resultGroup, where the result of your search is being displayed, is declared here.
	...
	/**
	 * Builds the appropriate widgets for the search group.
	 * A GridLayout is being used for this group having 11 columns.
	 * 
	 * @param Group
	 */
	...
	/**
	 * Builds the appropriate widgets for the result group.
	 * A GridLayout is being used for this group having 7 columns.
	 * 
	 * @param Group
	 */
	...
	private void fillFields() {		
		...
		fromDateText.setSelection(Tools.getFirstDate());
		toDateText.setSelection(Tools.getLastDate());
	}
	
	/**
	 * Reads all the textfields and combos and enters the value into a "SuchBuchung" object.
	 * If there are no data inputs a empty SuchBuchung is returned.
	 * 
	...
	private SearchBuchung readFields() throws RuntimeException {
		...
		if ((fromAccountCombo.getCombo().getSelectionIndex() > 0) && (toAccountCombo.getCombo().getSelectionIndex() > 0)) {
			suchBuchung.setSameAccount(fromAccountCombo.getSelectedAccount().getKonto() == toAccountCombo.getSelectedAccount().getKonto());
		}
		
		GregorianCalendar fromDateCal = null;
		if (fromDateText.getSelection() != null) {
			fromDateCal = new GregorianCalendar();
			fromDateCal.setTime(fromDateText.getSelection());
			...
			suchBuchung.setFromDate(fromDateCal.getTime());
		}
		
		GregorianCalendar toDateCal = null;
		if (toDateText.getSelection() != null) {
			toDateCal = new GregorianCalendar();
			toDateCal.setTime(toDateText.getSelection());
			...
			suchBuchung.setToDate(toDateCal.getTime());
		}
	...
	/**
	 * Refreshes the search if one was executed already in case
	 * a booking was changed somewhere else.
	 */
	@Override
	public void update() {
	...
	/**
	 * Makes this dataframe active. sets the focus to the booking text field
	 * and sets the application's default button to the search button.
	 */
	@Override
	...
	/**
	 * Called from the fastsearch. Clears the old search entries, enters the searchtext
	 * into the booking text field and searches.
	 * @param String
	 */
	 ...
	 * Updates the resultgroup then afterwards.
	 */
	private void edit() {
