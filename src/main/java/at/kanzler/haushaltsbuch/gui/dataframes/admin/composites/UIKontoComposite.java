package at.kanzler.haushaltsbuch.gui.dataframes.admin.composites;
...
import at.kanzler.haushaltsbuch.logic.admin.composites.LogicKontoComposite;
...
public class UIKontoComposite extends UIAbstractComposite implements SelectionListener {
    ...
    private Label ktobezTitle, kuerzelTitle, abldatTitle, ktotypTitle, kzogTitle, saldoTitle, jreinsaldoTitle,
            messageLabel;
    ...
    private void buildViewComposite() {
        ...
        editButton.setImage(
                new Image(this.getDisplay(), this.getClass().getClassLoader().getResourceAsStream("images/edit.png")));
    ...
    private void fillFields() {
        if (!isNew) {
            if (inUse) {
                ...
                abldatText.setSelection(Tools.toDate(konto.getAbldat()));
    ...
    private void readFields() throws RuntimeException {
        ...
        if (!ktobezText.getText().equals("")) {
            ...
            throw new RuntimeException(res.getString("EmptyTitle"));
        if (!kuerzelText.getText().equals("")) {
            konto.setKuerzel(kuerzelText.getText());
        } else
            throw new RuntimeException(res.getString("EmptyShortTitle"));
        if (logic.getKtotypSelection(ktotypCombo.getSelectionIndex()) != null) {
            ...
            throw new RuntimeException(res.getString("EmptyType"));
        if (logic.getKzogSelection(kzogCombo.getSelectionIndex()) != null) {
            ...
            throw new RuntimeException(res.getString("EmptyStatus"));
        }
        if (abldatText.getSelection() != null) {
            LocalDate abldat = Tools.toLocalDate(abldatText.getSelection());
            if (abldat.isAfter(LocalDate.now()) || Boolean.TRUE.equals(isNew()) == false) {
                konto.setAbldat(abldat);
            } else {
                throw new RuntimeException(res.getString("PastClosingDate"));
            }
        } else {
            throw new RuntimeException(res.getString("EmptyClosingDate"));
        }
    }
